...
def train_test_split_data(shuffled=True):
    ...
    if shuffled:
        filenames= np.load('PIE_dataset/train_val_filenames_shuffled.npy')
        labels= np.load('PIE_dataset/train_val_labels_shuffled.npy')
    else:
        filenames= np.load('PIE_dataset/train_val_filenames.npy')
        labels= np.load('PIE_dataset/train_val_labels.npy')
    ...
    np.save('PIE_dataset/train_filenames.npy', train_filenames)
    np.save('PIE_dataset/train_labels.npy', labels_train)

    np.save('PIE_dataset/val_filenames.npy', val_filenames)
    np.save('PIE_dataset/val_labels.npy', labels_val)
...
def image_pre_process(data,data_type='train',replace=False,save_shuffled=True):
    image_list=[]
    i = 0 
    name_array=[]
    label_array=[]
    if not os.path.exists('PIE_dataset/train_val_filenames') or replace:  # check if files are already pre-processed
        if replace:
            print('Replacing Previous Data...')
        else:
            print('No Previous Data Found')
        print('Image Pre_Processing started ...')
        img_save_folder = os.path.join('PIE_dataset/images')
        ...
        if(data_type=='train'): 
            np.save('PIE_dataset/train_val_filenames.npy',name_array)
            np.save('PIE_dataset/train_val_labels.npy',label_array)
        else:
            np.save('PIE_dataset/test_filenames.npy',name_array)
            np.save('PIE_dataset/test_labels.npy',label_array)

        if(save_shuffled):
            shuffling(name_array,label_array,data_type)
...
def shuffling(filenames,y_labels_one_hot,data_type):
    ...
    if (data_type == 'train'):
         np.save('PIE_dataset/train_val_labels_shuffled.npy', y_labels_one_hot_shuffled)
         np.save('PIE_dataset/train_val_filenames_shuffled.npy', filenames_shuffled)
    else:
         np.save('PIE_dataset/test_labels_shuffled.npy', y_labels_one_hot_shuffled)
         np.save('PIE_dataset/test_filenames_shuffled.npy', filenames_shuffled)
...
#"---------------------------------------------------------------------------------------------------------------------------"        
        ...
        print('Initail Data: ')
        data_size(beh_seq_train, print_details=False)
        # Flatten data to handle single images instead of image sequences
        beh_seq_train = flatten(beh_seq_train)
        print('After flattening: ',len(beh_seq_train['intention_binary']))
        beh_seq_train = data_balance(beh_seq_train, label_type='intention_binary')

        image_pre_process(beh_seq_train,data_type='train',replace=False,save_shuffled=True)   # replace to replace current data , save shuffled data to save shuffled data
        train_test_split_data(shuffled=False)
#"---------------------------------------------------------------------------------------------------------------------------"
        ...
        image_pre_process(beh_seq_test,data_type='test',replace=False,save_shuffled=True)   # replace to replace current data , save shuffled data to save shuffled data
